...
from keras_unet_models import wuunet
...
MODEL_NAME = '{}_{}_{}'.format('wuunet_light' if IS_LIGHT else 'wuunet', INPUT_SIZE, COMB_TYPE)
...
if __name__ == '__main__':
    ...
    model = wuunet(n_class=3, light=IS_LIGHT, dilation_rate=dilation_rate)
    model.compile(
        optimizer=optimizer,
        metrics={
            'out_bin':  bin_jaccard_acc,
            'out_mult': jaccard_acc
        },
        loss={
            'out_bin':  bce_bin_jaccard_loss,
            'out_mult': bce_jaccard_loss
        }
    )

    training_callbacks = [
        keras.callbacks.ModelCheckpoint(
            filepath=OUTPUT_PATH + '/snapshots/best_val_bin_loss_{epoch}.hdf5',
            monitor='val_out_bin_loss',
            save_best_only=True,
            verbose=1
        ),
        keras.callbacks.ModelCheckpoint(
            filepath=OUTPUT_PATH + '/snapshots/best_val_bin_acc_{epoch}.hdf5',
            monitor='val_out_bin_bin_jaccard_acc',
            save_best_only=True,
            verbose=1
        ),
        keras.callbacks.ModelCheckpoint(
            filepath=OUTPUT_PATH + '/snapshots/best_val_mult_loss_{epoch}.hdf5',
            monitor='val_out_mult_loss',
            save_best_only=True,
            verbose=1
        ),
        keras.callbacks.ModelCheckpoint(
            filepath=OUTPUT_PATH + '/snapshots/best_val_mult_acc_{epoch}.hdf5',
            monitor='val_out_mult_jaccard_acc',
            save_best_only=True,
            verbose=1
        ),
